...
$results = $db->fetchAll('SELECT DISTINCT SITE_NOMBRE FROM (SELECT SITE_NOMBRE FROM DVL_U2000_NE WHERE (NE_TYPE LIKE \'%RTN%\' AND SITE_NOMBRE IS NOT NULL) AND GATEWAY_TYPE = \'Gateway\' UNION SELECT SITE_NOMBRE FROM DVL_AGG UNION SELECT SITE_NOMBRE FROM DVL_PAG WHERE HOSTNAME LIKE \'%PAG%\' AND ESTADO = \'ON-Air\') ORDER BY SITE_NOMBRE');
for ($i = 0 ; $i < count($results) ; $i++)  $gateway_site[] = $results[$i]['SITE_NOMBRE'];

$results = $db->fetchPairs('SELECT NE_NAME,SITE_NOMBRE FROM DVL_U2000_NE WHERE NE_TYPE LIKE \'%RTN%\' AND SITE_NOMBRE IS NOT NULL');
$ne_site = $results;
...
for($t = 3 ; $t < count($worksheet) ; $t++) {
    ...
    if(!in_array($worksheet[$t][3],$tunneles_disponibles))
        continue;
...
for($i = 0 ; $i < count($tunneles_sin_sentido) ; $i++) {
    ...
    if($know_exists) {
        ...
        $temp_gateways = ['0100613_LI_Chicama to Cartavio','0101009_LA_Picsi to Moshoqueque','0101711_PI_El_Alto to Lobitos','0101311_CS_Churcana to Salida Churcana','0102325_SM_Juanjui to Sacanche','0103675_CS_Quillabamba_Ciudad to Wanchaq','0105452_LM_Unger to Rosa de America'];
